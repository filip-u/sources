...
export default function App() {
  ...
  const [courseEntryScreen, setCourseEntryScreen] = useState(() => {
    try { return localStorage.getItem("ff_course_entry_screen") === "1"; } catch { return false; }
  });
  ...
  const [currentPromptButton, setCurrentPromptButton] = useState(() => {
    try { return localStorage.getItem("ff_current_prompt_button") === "1"; } catch { return false; }
  });
  ...
  function toggleCourseEntryScreen() {
    setCourseEntryScreen((value) => {
      const next = !value;
      try { localStorage.setItem("ff_course_entry_screen", next ? "1" : "0"); } catch { /* ignore */ }
      return next;
    });
  }
  ...
  function toggleCurrentPromptButton() {
    setCurrentPromptButton((value) => {
      const next = !value;
      try { localStorage.setItem("ff_current_prompt_button", next ? "1" : "0"); } catch { /* ignore */ }
      return next;
    });
  }
  ...
  function openSourceLibrariesGrid() {
    ...
    try {
      localStorage.setItem("ff_multi_library", "1");
      localStorage.setItem("ff_source_libraries", "1");
    } catch { /* ignore */ }
